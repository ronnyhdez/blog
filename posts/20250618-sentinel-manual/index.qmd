---
title: "Getting Started with Sentinel-2 Downloading, Processing, and Analyzing Satellite Imagery with Python" 
description: "A hands-on tutorial for downloading Sentinel-2 imagery from Copernicus, understanding the file structure, and processing RGB bands and vegetation indices using Python."
date: 2025-06-18
...
just want to obtain one image to explore the necessary steps to understand the
files, metadata, transform the pixels and visualize the image.
...
![Copernicus Data Space browser interface showing the main search page](img/copernicus_1.png)
...
![Copernicus Data Space parameters tab showing filter options for satellite imagery search](img/copernicus_2.png)
...
For this example we just want to work with one image and not several of them
(which can imply merging them in a separate process)

Your search process can look something like:

![Search results showing available Sentinel-2 images with their metadata and preview](img/copernicus_3.png)

In my case, I wanted a clear image with atmospheric correction, so I proceeded to
download the `L2A` product.

![Download options showing L2A and L1C products with their file names and sizes](img/copernicus_4.png)

You can see the names are quite long. There's a reason for this: the filename provides
key information about the product we're using.
...
![Directory structure of a Sentinel-2 L2A product showing GRANULE folder and metadata files](img/contents.png)

The images are contained in the `GRANULE` folder. Inside this one we will find
...
Now that we understand what the contents are of the product we downloaded, we
...
explore how they looked on March 3rd, 2024 (Satellite image date)
...
relative path, so later I can use just the object name, and save a lot of
typing.
...
to the file without loading its content into memory (lazy loading). That way
...
if the image is so big that we run out of memory. Be aware!
...
We'll need some processing to make it look nice 
...
plt.show()
```
Here's another option to improve the image. This is purely for visualization purposes
to make the landscape more visually appealing—it doesn't have any research application
at this stage. 
...
do much more and take advantage of the rest of the bands we have within the data
...
We created the visualization above using the RGB bands at a resolution of 10m,
